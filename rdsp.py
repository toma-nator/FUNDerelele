...
def project(start_year: int, start_value_cents: int, birth_year: int, *,
    ...
    for year in range(start_year, end_year + 1):
        ...
        requested_amt = dap_rejected = 0      # ldap_dap: what was asked vs. clipped by the cap
        pgap = False
        if withdrawal and year >= withdrawal['start_year']:
            ...
            if mode == 'max':                              # take the efficient legal max each year
                ...
                requested_amt = ldap_amt + max(0, dap)     # the full ask, before the PGAP cap
                withdrawal_amt = min(requested_amt, cap)
            else:                                          # 'ldap'
            ...
            if requested_amt:                              # how much of the ask the caps rejected
                dap_rejected = max(0, requested_amt - withdrawal_amt)
        ...
        rows.append({
            ...
            'growth': growth, 'withdrawal': withdrawal_amt, 'dap_rejected': dap_rejected,
